...
if(isset($_POST['delete'])){
	$delId = $_GET['id'];
	$query = "DELETE FROM `crime_list` WHERE id = '$delId'";
...
$query = "INSERT INTO `crime_list`(`name`) VALUES ('$name')";
$result = $conn->query($query);
if($result){
	$_SESSION['message'] = "Crime Added";
	header('Location:crimes.php');
}else{
	$_SESSION['message'] = "Unable to Add Crime";
	header('Location:crimes.php');
}
} 
?>
...
<div class="w3-bar">
  <a href="#" class="w3-button w3-white w3-hover-white"><b>List of Crimes</b></a> 
...
<div class="w3-card table">
			<table id="list" class="w3-table w3-striped w3-bordered w3-hoverable">
				...
				<?php 
					...
					<div class="dropdown">
						...
						<a href="edit_crime.php?id=<?= $row['id']?>" class="logbtn w3-button w3-hover-light-grey">Edit</a>
						<form action="crimes.php?id=<?= $row['id']?>" method="POST">
	...
	<div id="id01" class="modal"> 
		<form class="modal-content animate" action="crimes.php" method="POST"> 
			<h2><center>Crime Registration</center></h2>
		<div class="w3-row" class="sob">
 
			<label>Name</label></br>
			<input type="text" name="crime" required> </br>
			</br>
	    </div></br></br>
